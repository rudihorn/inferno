...
let annotation_to_structure ((qs, body) : O.scheme) : Lo.variable S.structure =
  ...
  match qs' with
  | [] -> O.to_structure (fun s -> Lo.fresh (Some s)) env body
  | _  -> S.forall qs' (O.to_variable (fun s -> Lo.fresh (Some s)) env body)
